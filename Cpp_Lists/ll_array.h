...
if (k < 1 || !first) throw length_error("No space in list to add new element in Insert"); // no k'th
...
q = q->link;
if (!q || !q->link) throw out_of_range("Invalid element location in Insert"); // no k'th
...
if (k < 0) throw length_error("No space in list to add new element in Insert");
// p will eventually point to k'th node
ChainNode<T> *p = first;
for (int index = 1; index < k && p; index++)
p = p->link; // move p to k'th
if (k > 0 && !p) throw out_of_range("Invalid element location in Insert"); // no k'th
// insert
